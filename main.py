...
from scraper import fetch_all_rss, is_potentially_price_related, scrape_press_page
...
def run_pipeline(notify: bool = True) -> None:
    ...
    for item in headlines:
        ...
        # Tani filtr słów kluczowych przed kosztownym wywołaniem AI
        if not is_potentially_price_related(full_text):
            skipped_by_filter += 1
            continue
    ...
    print(f"Pominięto (filtr słów kluczowych): {skipped_by_filter}")
