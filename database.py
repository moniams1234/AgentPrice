...
def log_token_usage(
    ...
    """
    Zapisuje zużycie tokenów dla jednego wywołania API.

    process: nazwa czynności, np. "ekstrakcja_automatyczna",
             "ekstrakcja_reczna", "ocena_wplywu_marze"
    """
    total = input_tokens + output_tokens + reasoning_tokens
    with get_connection() as conn:
        conn.execute(
            """INSERT INTO token_usage
            (process, model, input_tokens, output_tokens, reasoning_tokens,
             total_tokens, logged_at)
            VALUES (?, ?, ?, ?, ?, ?, datetime('now'))""",
            (process, model, input_tokens, output_tokens, reasoning_tokens, total),
        )


def get_token_usage_summary() -> list[dict]:
    """Zwraca sumaryczne zużycie tokenów pogrupowane wg procesu i modelu."""
    with get_connection() as conn:
        rows = conn.execute(
            """SELECT
                process,
                model,
                COUNT(*) AS wywolania,
                SUM(input_tokens) AS suma_wejsciowych,
                SUM(output_tokens) AS suma_wyjsciowych,
                SUM(reasoning_tokens) AS suma_myslenia,
                SUM(total_tokens) AS suma_calkowita
            FROM token_usage
            GROUP BY process, model
            ORDER BY suma_calkowita DESC"""
        ).fetchall()
        return [dict(r) for r in rows]


def get_token_usage_log(limit: int = 100) -> list[dict]:
    """Zwraca surowy log ostatnich wywołań (najnowsze pierwsze)."""
    with get_connection() as conn:
        rows = conn.execute(
            "SELECT * FROM token_usage ORDER BY logged_at DESC LIMIT ?",
            (limit,),
        ).fetchall()
        return [dict(r) for r in rows]
